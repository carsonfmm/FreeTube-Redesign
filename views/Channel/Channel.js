...
export default Vue.extend({
  ...
  methods: {
    ...
    async getAllChannelVideos(continuationStr, videoInformation, depth ) {
      let reversedList = [];
      let sortedList = [];
       ytch.getChannelVideosMore({continuation: continuationStr}).then((response) => {
        if (response.items != null) {
          for (let i = 0; i < response.items.length; i++) {
            videoInformation.push(response.items[i]);
          }
        }
        if (response.continuation == null) {
          if (this.videoSortBy == "newest") {
            this.latestVideos = videoInformation
            this.isElementListLoading = false
          }
          else if (this.videoSortBy == "oldest") {
            console.log(videoInformation)
            reversedList = videoInformation;
            reversedList.reverse();
            this.latestVideos = reversedList;
            this.isElementListLoading = false
          }
          else if (this.videoSortBy == "popular") {
            sortedList = videoInformation;
            sortedList.sort( this.compare )
            this.latestVideos = sortedList;
            this.isElementListLoading = false
          }
          return videoInformation;
        }
        else if (depth < 8) {
          return this.getAllChannelVideos(response.continuation, videoInformation, depth+1);
        }
        else {
          if (this.videoSortBy == "newest") {
            this.latestVideos = videoInformation
    ...
    getChannelVideosLocal: function () {
      ...
      if (this.videoSortBy != 'newest') {
        ytch.getChannelVideos({ channelId: this.id, sortBy: 'oldest', channelIdType: 0 }).then((response ) => {
          for (let i = 0; i < response.items.length; i++) {
            videoInformation.push(response.items[i]);
          }
          console.log("RESponSE: ", response)
          if (response.continuation != null) {
            videoInformation = this.getAllChannelVideos(response.continuation, videoInformation, 0);
          }
          else if (this.videoSortBy == 'oldest') {
            tempArray = videoInformation;
            tempArray.reverse();
            this.latestVideos = tempArray;
            this.isElementListLoading = false;
          }
          else if (this.videoSortBy == 'popular') {
            tempArray = videoInformation;
         ...
         //
         //THIS WAS THE OLD FUNCTION THAT WAS USED TO PULL VIDEOS AND DISPLAY ON THE CHANNEL'S PAGE
        //
