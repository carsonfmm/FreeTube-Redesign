...
var invidiusApis = require("../../assets/locationAPIs/locationLinks.json")
...
export default Vue.extend({
  ...
  computed: {
    ...
    countryValues: function () {
      var values = []
      for (var i = 0; i < invidiusApis.length; i++) {
        if (values.includes(invidiusApis[i][1].uri) == false) {
          if (invidiusApis[i][1].monitor != null){
            if (invidiusApis[i][1].monitor.statusClass == "success") {
              values.push(invidiusApis[i][1].uri)
            }
          }
        }
      }
    ...
    countriesAvalible: function() {
      var countries = []

      for (var i = 0; i < invidiusApis.length; i++) {
        if (countries.includes(invidiusApis[i][1].region + " " + invidiusApis[i][1].flag) == false) {
          if (invidiusApis[i][1].monitor != null){
            if (invidiusApis[i][1].monitor.statusClass == "success") {
              countries.push(invidiusApis[i][1].region + " " + invidiusApis[i][1].flag)
            }
          }
        }
      }
  ...
  methods: {
    handleInvidiousInstanceInput: function (input) {
      ...
      var isSet =false
      for (var i = 0; i < invidiusApis.length; i++) {
        if (invidiusApis[i][1].uri == instance) {
          isSet = true
          this.country = invidiusApis[i][1].region + " " + invidiusApis[i][1].flag
          this.setCurrentInvidiousInstance(instance)
        }
      }

      if (!isSet) {
        this.setCurrentInvidiousInstance = "Unknown Country"
      }
    },
