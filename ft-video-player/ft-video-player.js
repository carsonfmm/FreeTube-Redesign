...
export default Vue.extend({
  ...
  methods: {
    ...
    updateVolume: function (_event) {
      ...
      if (this.$store.getters.getRememberDefaultVolume == true) {
    ...
    changeVolume: function (volume) {
      ...
      if (newVolume < 0) {
        this.player.volume(0)
        if (this.rememberDefaultVolume) {
          updateDefaultVolume(0)
          this.defaultVolume = 0
        }
      } else if (newVolume > 1) {
        this.player.volume(1)
        if (this.rememberDefaultVolume) {
          updateDefaultVolume(1)
          this.defaultVolume = 1
        }
      } else {
        this.player.volume(newVolume)
        if (this.rememberDefaultVolume) {
          updateDefaultVolume(newVolume)
